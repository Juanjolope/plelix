...
<div class="container mx-auto mt-12">
    ...
    <div class="text-center">
        <h1 class="text-4xl font-bold mb-8">PELICULAS EN CARTELERA</h1>
    </div>
...
<div class="mt-8">
    <div class="flex justify-center">
        @if($currentPage > 1)
        <a href="{{ route('cartelera', ['page' => $currentPage - 1]) }}" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 mr-2">Anterior</a>
        @endif

        @for($i = max(1, $currentPage - 5); $i <= min($currentPage + 5, $totalPages); $i++)
        <a href="{{ route('cartelera', ['page' => $i]) }}" class="bg-gray-200 text-gray-800 py-2 px-4 rounded hover:bg-gray-300 mx-1 {{ $currentPage == $i ? 'bg-blue-500 text-white' : '' }}">{{ $i }}</a>
        @endfor

        @if($currentPage < $totalPages)
        <a href="{{ route('cartelera', ['page' => $currentPage + 1]) }}" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 ml-2">Siguiente</a>
        @endif
    </div>
</div>
@endsection
